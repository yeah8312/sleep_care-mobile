...
- 워치 앱은 companion 세션 중계와 Samsung Health Sensor SDK 기반 실제 심박 backend를 구현했으며, 로컬 SDK AAR과 실기기 policy 검증이 필요함
- 규칙 기반 추천 엔진 추가
...
- 워치 모듈 빌드 시 `watch/libs/samsung-health-sensor-api.aar` 로컬 배치 필요
...
워치 모듈까지 포함한 `testDebugUnitTest`는 Samsung Health Sensor SDK AAR을 로컬에 배치한 뒤 실행해야 합니다.
...
- 워치 앱은 Samsung Health Sensor SDK 실센서 backend를 사용하므로, AAR 미배치 또는 SDK policy/package/signing 불일치 시 워치 빌드나 센서 시작이 실패할 수 있음
...
- 모바일 앱 ↔ 워치 앱: companion 세션/큐/커서/진동 경고 구조와 Samsung Health Sensor SDK 실센서 backend 구현
